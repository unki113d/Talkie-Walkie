...
public class RagdollController : NetworkBehaviour
{
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        float upDot = Vector3.Dot(n, Vector3.up);     // +1 — пол, 0 — стена, -1 — потолок

        // 1) игнор пола/потолка — это не «сбили»
        if (upDot > 0.5f || upDot < -0.5f)
            return;

        // 2) скорость влетания ВДОЛЬ нормали (а не модуль)
        // отрицательная — уезжаем от стены, положительная — влетаем
        ...
        // (опционально) очень редкий кейс — мощный наклонный удар
